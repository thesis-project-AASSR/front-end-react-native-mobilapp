  ...
  {Object.entries(profile).filter(([key, y]) => 
      y.userID == user).map(([key,v]) => {
          ...
          <Text>phone number: {v.phoneNumber}</Text>
           <Button onPress={() => navigation.navigate(('update profile') ,{id : v.userID, username:v.username,email:v.email,phoneNumber:v.phoneNumber,image:v.image,location:v.location} )} title="update profile"/>
          </Text>
                 </View>
      })}
   </View>
